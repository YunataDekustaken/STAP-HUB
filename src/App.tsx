...
export default function App() {
  ...
  const [isAccessRequestTab, setIsAccessRequestTab] = useState<boolean>(false);
          ...
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl border border-slate-100 p-6 space-y-5 text-left animate-scaleIn">
            <div className="text-center space-y-1.5">
              ...
              <h3 className="text-base font-bold text-slate-800">
                {isAccessRequestTab ? "Request STAP Access" : "STAP Operator Authentication"}
              </h3>
              <p className="text-xs text-slate-400 font-semibold">
                {isAccessRequestTab 
                  ? "Submit your details to request system operator access" 
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl mb-4">
              <button 
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${!isAccessRequestTab ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                onClick={() => setIsAccessRequestTab(false)}
              >
                Sign In
              </button>
              <button 
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${isAccessRequestTab ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                onClick={() => setIsAccessRequestTab(true)}
              >
                Request Access
              </button>
            </div>
              ...
              {isAccessRequestTab ? (
                <form 
                  onSubmit={(e) => {
                    e.preventDefault();
                    setLoginError("");
                    alert("Access request submitted to system administrators. You will be notified via email upon approval.");
                    setIsAccessRequestTab(false);
                  }}
                  className="space-y-3"
                >
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Full Name</label>
                    <input type="text" required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs outline-none focus:border-[#4E6290]" placeholder="Officer Name" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Google Email Address</label>
                    <input type="email" required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs outline-none focus:border-[#4E6290]" placeholder="officer@example.com" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Reason for Access</label>
                    <textarea required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs outline-none focus:border-[#4E6290] resize-none h-16" placeholder="Brief explanation of your role..."></textarea>
                  </div>
                  <button type="submit" className="w-full py-3 px-4 bg-[#4E6290] hover:bg-[#3D4F75] text-white font-bold rounded-xl text-xs transition-all cursor-pointer text-center mt-2 shadow-sm">
                    Submit Request
                  </button>
                </form>
              ) : (
                <>
                  {getFirebaseConfig().connected ? (
                    <button
                      type="button"
                      onClick={async () => {
                        const { auth, provider } = getFirebaseInstances();
                        if (auth && provider) {
                          try {
                            setLoginError("");
